...
// void export_ul_ch_estimates(c16_t **ul_ch_estimates, int num_antennas_rx, int symbolSize, int ch_offset) {
//     static int file_count = 0; // 用于计数生成的文件数量
//     char filename[100];
...
//     snprintf(filename, sizeof(filename), "ul_ch_estimates_%d.txt", file_count);
//     FILE *file = fopen(filename, "w");
//     if (file == NULL) {
//         printf("Error opening file!\n");
//         return;
...
//     for (int i = 0; i < num_antennas_rx; i++) {
//         c16_t *ul_ch = &ul_ch_estimates[i][ch_offset];
//         for (int j = 0; j < symbolSize; j++) {
//             // 计算平方范数
//             double norm_squared = (ul_ch[j].r * ul_ch[j].r) + (ul_ch[j].i * ul_ch[j].i);
//             fprintf(file, "%.6f\n", norm_squared);
//         }
//     }
                                ...
                                uint32_t *nvar)
{
  ...
  // 导出 ul_ch_estimates 数组
  export_ul_ch_estimates(ul_ch_estimates, gNB->frame_parms.nb_antennas_rx, symbolSize, ch_offset);
